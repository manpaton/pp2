...
back = p.image.load("snake_fon.jpg")
...
# Setting up buttons
qquit = p.image.load("quit.png")
qquit = p.transform.scale(qquit, (125, 102))
restart = p.image.load("restart.png")
    ...
    if start_screen:
        ...
        for i in p.event.get():
            ...
            if i.type == p.MOUSEBUTTONDOWN and collision:
                ...
                if restart_rect.collidepoint(mouse_pos):
                    p.mixer.Sound("mouse_click.mp3").play()
                    time.sleep(0.5)
                    score = 0
                    SPEED = 5
                    level = 1
        ...
        if not collision and not pause:
            ...
            if (
                ...
                p.mixer.Sound("sound_apple.mp3").play()
                food_spawn_time = time.time()
                if food == food_size1:
                    score += 3
                ...
                if level - prev_level >= 1:
                    p.mixer.Sound("level_up.mp3").play()
                    prev_level = level
            else:
                snake_body.pop()
